package org.liws.framework.util.serialize;
...
public class KryoSerializeUtil {
    ...
    static {
        factory = new KryoFactory() {
			...
			private void doRegister(Kryo kryo) {
        ...
        try {
            ...
            try (Input input = new Input(inputStream)) {
                try {
                    ...
                    input.setPosition(0);
                    try {
                        // TODO kryo.setClassLoader(SpringContextHolder.getClassLoaderByContextName("/console"));
                        return kryo.readClassAndObject(input);
                    } catch (com.esotericsoftware.kryo.KryoException ex2) {
                        input.setPosition(0);
                        try {
                        	// TODO kryo.setClassLoader(SpringContextHolder.getClassLoaderByContextName("/bq_self"));
                            return kryo.readClassAndObject(input);
                        } catch (com.esotericsoftware.kryo.KryoException ex3) {
                            input.setPosition(0);
                            try {
                            	// TODO kryo.setClassLoader(SpringContextHolder.getClassLoaderByContextName("/di"));
                                return kryo.readClassAndObject(input);
